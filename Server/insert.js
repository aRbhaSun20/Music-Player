...
datas.map((data, index) => {
	sql = `insert into artistData (artist_name) values ("demo ${index}")`;
	connection.query(sql, (err, res) => {
		if (err) throw err;
	});
	sql = `insert into albumData (album_name) values ("demo ${index}")`;
	connection.query(sql, (err, res) => {
		if (err) throw err;
	});
	sql = `insert into playlistData (playlist_name) values ("demo ${index}")`;
	connection.query(sql, (err, res) => {
		if (err) throw err;
	});
