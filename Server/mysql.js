...
io.on("connection", (socket) => { // db connections
    const musicConnection = mysql.createConnection({host: process.env.HOST, user: process.env.USER, password: process.env.PASSWORD, database: process.env.DATABASE});

    const fetchData = () => { // fetch the complete music data
    ...
    const fetchUserData = () => { // fetch the user data
    ...
    socket.on("signUpUser", (data) => {
        let insertUser = `insert into user (user_name,membershipStatus,email,password) values ('${
            data.Name
        }','false','${
            data.Email
        }','${
    ...
    socket.on("loginUser", (data) => {
        let logInUser = `insert into loginData (email,password) values ('${
            data.Email
        }','${
            data.Password
        }' )`;
    ...
    socket.on("feedbackData", (data) => {
        let feedbackData = `insert into feedbackForm (name,email,phone,message) values ('${
            data[0]
        }','${
            data[1]
        }','${
    ...
    socket.on("changeDetails", (data) => {
        let changeData = `update into songData (name,email,phone,message) values ()`;
        musicConnection.query(changeData, (err, res) => {
            if (err) 
                throw err;
    ...
    socket.on("deleteData", (data) => {
        let deleteSongData = `delete from songData where song_id = ${
            data[0]
        }`;
        let deleteBrowseData = `delete from browseData where song_id = ${
            data[0]
        }`;
        let deleteRecentData = `delete from recentData where song_id = ${
            data[0]
        }`;
